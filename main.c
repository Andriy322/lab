...
void swap(student* b1, student* b2);
void delete(student* b1);
int main()
{
	FILE* fp;
	fp = NULL;
	char* filename[45];
	do {
		//printf("Enter file name(.txt): "); //C:\Andriy\Cprogram\lab10\laba10.txt
		strcpy(filename, "C:\\Andriy\\Cprogram\\lab10\\laba10.txt");
		//filename[45] = "C:\\Andriy\\Cprogram\\lab10\\laba10.txt";
	...
	for (int i = 0; i < nOfNodesInFile; i++) {
		...
		struct student* pp = (student*)malloc(sizeof(student));
		studic->mNext = pp;
		studic = pp;
	}
	studic->mNext = NULL;
	...
	scanf("%s", &add->person.surname);
	printf("Enter name of new student:");
	scanf("%s", &add->person.name);
...
void swap(student* b1, student* b2) {
	student bp;
	memcpy(bp.person.name, b1->person.name, 50);
	memcpy(bp.person.surname, b1->person.surname, 50);

	memcpy(b1->person.name, b2->person.name, 50);
	memcpy(b1->person.surname, b2->person.surname, 50);

	memcpy(b2->person.name, bp.person.name, 50);
	memcpy(b2->person.surname, bp.person.surname, 50);


	bp.birthday.day = b1->birthday.day;
	bp.birthday.month = b1->birthday.month;
	bp.birthday.year = b1->birthday.year;

	b1->birthday.day = b2->birthday.day;
	b1->birthday.month = b2->birthday.month;
	b1->birthday.year = b2->birthday.year;

	b2->birthday.day = bp.birthday.day;
	b2->birthday.month = bp.birthday.month;
	b2->birthday.year = bp.birthday.year;

	for (int i = 0; i < 5; i++)
	{
		bp.marks[i] = b1->marks[i];
		b1->marks[i] = b2->marks[i];
		b2->marks[i] = bp.marks[i];
	}
}
int check(int arr[])
{
